...
Token Lexer::lex() {
	...
	switch (const char st = input.take()) {
		...
		case '0': return token(Number); // TODO: octal support
		...
		case '"': {
			while (true) {
				switch (const char c = input.take()) {
					case -1: throw_with_msg("Unterminated string");
					case '\\': input.take(); break;
					default:
...
#undef bop
		...
		case '>': {
			...
			else { input.rewind(); return token(right? Gt: Lt); }
			break;
		}
		case '=': {
			if (input.peek() == '=') {
		...
		case '&': {
			...
			else { input.rewind(); return token(ad? And: Or); }
			break;
		}
		case '!': {
			if (input.peek() == '=') {
		...
		case '/': {
			switch (input.take()) {
				...
				default: input.rewind(); return token(Div);
			}
		}
